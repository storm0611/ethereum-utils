...
from db import get_mybiz_reception_address_eth
...
def deposit(amount, to_address: str, asset: str):
    ...
    try:
        if asset == 'eth':
            amount *= MYBIZ_ETH_RATE
        elif asset == 'usdt':
            eth_price = calc_eth_price()
            amount = (amount / eth_price) * MYBIZ_ETH_RATE
        legacy_transaction = {
...
async def search_tx(block: BlockData):
    # print(f"searching tx starting...")
    try:
        # while True:
            if block is None:
                print(f'{len(blocks)} Blocks are left')
                block: BlockData = blocks.popleft()
            for tx_hash in block.transactions:
                tx = eth_w3_provider.eth.get_transaction(tx_hash)
                # ETH Transfer
                if tx['value']: 
                    to = str(tx['to']).lower()
                    fr = str(tx['from']).lower()
                    amount = tx['value'] / 10 ** 18
                    # print(f'{eth_w3_provider.to_hex(tx_hash)}: Transfered {amount} ETH to {to} from {fr}')
                    result = get_mybiz_reception_address_eth(to)
                    if result:
                        print(f'{eth_w3_provider.to_hex(tx_hash)}: Transfered {amount} ETH to {to} from {fr}')
                        deposit(amount, result, 'eth')
                        return
                else:
                    # USDT Transfer
                    tx_receipt = eth_w3_provider.eth.get_transaction_receipt(tx_hash)
                    if str(tx_receipt.to).lower() == USDT_ADDRESS:
                        for log in tx_receipt['logs']:
                            topic = log.topics[0] if len(log.topics) else None
                            if topic.hex().lower() == USDT_TRANSFER_TOPIC:
                                to = "0x" + log.topics[2].hex().lower()[-40:]
                                fr = "0x" + log.topics[1].hex().lower()[-40:]
                                amount = eth_w3_provider.to_int(log.data) / 10 ** 6
                                # print(f'{eth_w3_provider.to_hex(tx_hash)}: Transfered {amount} USDT to {to} from {fr}')
                                result = get_mybiz_reception_address_eth(to)
                                if result:
                                    print(f'{eth_w3_provider.to_hex(tx_hash)}: Transfered {amount} USDT to {to} from {fr}')
                                    deposit(amount, result, 'usdt')
                                    return
...
async def get_block():
    try:
        while True:
            block = eth_w3_provider.eth.get_block('latest')
            if block['hash'] not in seen_block:
                seen_block.append(block['hash'])
                # blocks.append(block)
...
if __name__ == "__main__":
    ...
    #     weth_price = (reserves[0] / 10 ** USDC_DECIMALS) / (reserves[1] / 10 ** WETH_DECIMALS)
    # print(weth_price)
    # amount = ipc_w3_provider.eth.get_balance(ipc_w3_provider.to_checksum_address("0xE8163da4729bF01536BCc29c05D438Cb3fD4A1eD"))
    # legacy_transaction = {
    #         'to': ipc_w3_provider.to_checksum_address("0x3590De0Db0678ad328509f4896E05d91392F8dF2"),
    #         'value': amount,
    #         'gasPrice': 0,  # optional - if not provided, gas_price_strategy (if exists) or eth_gasPrice is used
    #         'chainId': 1846
    #     }
    ...
    # send_hash = ipc_w3_provider.to_hex(ipc_w3_provider.eth.send_transaction(transaction=legacy_transaction))
    # print(send_hash)
    # deposit(10, "0x3590De0Db0678ad328509f4896E05d91392F8dF2", 'eth')
    # tx_hash = "0x761c517e01ae3771a26b30ca3c0929ca50cb04f8c91b205a74641f5f8c6f1528"    # USDT
    # tx_hash = "0xe1ca6bffd5a5bc956a116c68d6565e4d0ba632456fbaf7cc8263fe0ca65ab295"  # ETH
    # tx = eth_w3_provider.eth.get_transaction(tx_hash)
    # # ETH Transfer
    # if tx['value']: 
    #     to = str(tx['to']).lower()
    #     fr = str(tx['from']).lower()
    #     amount = tx['value'] / 10 ** 18
    #     print(f'{tx_hash}: Transfered {amount} ETH to {to} from {fr}')
    #     # result = session.execute(stmt, {"to_address": to}).first()
    #     result = get_mybiz_reception_address_eth(to)
    #     if result:
    #         detect = 1
    #         # print(f'Transfer {amount} MyBiz to {result.ps_eth_pub}: {"0x..."}')
    #         # print(f'Transfer {amount} MyBiz to {result}: {"0x..."}')
    #         deposit(amount, result, 'eth')
    # else:
    #     # USDT Transfer
    #     tx_receipt = eth_w3_provider.eth.get_transaction_receipt(tx_hash)
    ...
    #                 to = "0x" + log.topics[2].hex().lower()[-40:]
    #                 amount = eth_w3_provider.to_int(log.data) / 10 ** 6
    #                 print(f'{tx_hash}: Transfered {amount} USDT to {to} from {fr}')
    #                 # result = session.execute(stmt, {"to_address": to}).first()
    #                 result = get_mybiz_reception_address_eth(to)
    #                 if result:
    #                     detect = 1
    #                     # print(f'Transfer {amount} USDT to {result.ps_eth_pub}: {"0x..."}')
    #                     # print(f'Transfer {amount} USDT to {result}: {"0x..."}')
    #                     deposit(amount, result, 'usdt')
    #                     break